...
def reverse2wholeimage(dest_img_align_crop_tenor_list, swaped_imgs, mats, crop_size, dest_img, pasring_model =None, norm = None, use_mask = False):
    ...
    for swaped_img, mat, source_img in zip(swaped_imgs, mats, dest_img_align_crop_tenor_list):
        ...
        orisize = (dest_img.shape[1], dest_img.shape[0])
        if use_mask:
    ...
    # target_image /= 255
    # target_image = 0
    img = np.array(dest_img, dtype=np.float)
